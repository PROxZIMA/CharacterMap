...
var fileButton, fontFamily, search, selectedFont;
...
$(document).ready(function () {
  ...
  search.addEventListener('input', debounce(onTextChange, 275), false);
  var fontFileName = 'public/fonts/arialbd.ttf';
  opentype.load(fontFileName, function (err, font) {
    ...
    font.glyphs.glyphs = Object.values(font.glyphs.glyphs);
    selectedFont = font;
    onFontLoaded(font);
  });
  prepareGlyphList();
...
function renderGlyphItem(canvas, glyphIndex) {
  ...
  var glyph = font.glyphs.glyphs[glyphIndex],
    ...
    x0 = xmin;
  ctx.fillText(String.fromCharCode(glyph.index), cellWidth - 8, cellHeight - 2);
...
function onTextChange(e) {
  ...
  if (!searchTerm.trim()) {
    onFontLoaded(selectedFont);
    return
  }

  font = _.cloneDeep(selectedFont);
  font.glyphs.glyphs = selectedFont.glyphs.glyphs.filter(
    glyph =>
      (glyph.name || '').toLowerCase().includes(searchTerm) ||
      (glyph.unicodes || []).map(formatUnicode).join(' ').toLowerCase().includes(searchTerm)
  );

  if (font.glyphs.glyphs.length === 0) {
    font.glyphs.glyphs.push(selectedFont.glyphs.glyphs[0]);
  }

  font.numGlyphs = font.glyphs.glyphs.length;
  onFontLoaded(font);
}
...
function displayGlyphData(glyphIndex) {
  ...
  html += '<div><dt>indexHEX</dt><dd>' + glyph.index.toString(16) + '</dd></div>';
